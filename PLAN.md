...
目前 **Phase 1、Phase 2、Phase 3、Phase 4、移动端适配与 Phase 6 的多 graph 基础能力已完成**，仓库已经落地了一个可运行的前端骨架、真实画布、只读 Markdown 面板、可操作工具栏、基础移动端单栏布局，以及多 graph 工作区，具体包括：
...
* 左侧已支持节点标题编辑、`新建节点 / 删除选中节点 / 设为跳转节点 / 取消跳转节点`
...
* 工作区数据已写入 `localStorage`，刷新后可恢复 graphs、节点、边、note 内容与节点位置
...
* 更复杂的图谱编辑（如 note 正文编辑、连线规则约束、edge 属性编辑）
* 视图状态持久化（如缩放级别、平移位置）
* 自动布局

因此，下面的内容仍然是**后续阶段规划**，但其中的 Phase 1、Phase 2、Phase 3、Phase 4、基础移动端适配，以及多 graph 工作区的第一版能力已经进入“已完成”状态。
...
4. 左侧提供基础操作：
   ...
   * 自动布局预留入口
   * 导入/导出 JSON
5. 节点和 Markdown 内容自动保存到 `localStorage`
6. Markdown 支持：
